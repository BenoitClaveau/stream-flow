...
class StreamWorflow extends Duplex {
    ...
    async init(chunk, encoding) {
        var ts = this._flowState;
        ts.output = await ts.initFn(ts.input, chunk, encoding);
        if (!ts.output) {
            this.emit("error", new Error("Output stream is not returned."));
            return;
        }
    ...
    async _write(chunk, encoding, callback) {
        var ts = this._flowState;
        if (!ts.init) await this.init(chunk, encoding);
